...
class DDNModelPlugin(PythonPlugin):
    ...
    def initConnectionParams(self, device):
        ...
        """
        user = getattr(device, 'zCommandUsername', 'essim')
        passwd = getattr(device, 'zCommandPassword', 'essim')
        targets = getattr(device, 'zESMdsNodes', '10.1.1.1')
        targets = targets.split(",")
        # Get PreferredTarget from device, if earlier model was successful
        preferredTarget = getattr(device, 'PreferredTarget', None)
    ...
    def collect(self, device, log):
        log.debug("%s: collection for device", device.id)
    ...
    def connectCallback(self, connection):
        ...
        """
        log.debug("XXX connectCallback with connection %r", connection)
